function computeOrdinalNumber(number: number): string {
    ...
    switch (numberToString[numberToString.length - 1]) {
        case "1":
            return `${number}${numberToString[numberToString.length - 2] === '1' ? 'th' : 'st'}`
        case "2":
            return `${number}${numberToString[numberToString.length - 2] === '1' ? 'th' : 'nd'}`
        case "3":
            return `${number}${numberToString[numberToString.length - 2] === '1' ? 'th' : 'rd'}`
        default:
            return `${number}th`
    }
}
